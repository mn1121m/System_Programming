/*
file:   tinybc.c
description:    a tiny calculator thar uses dc to do its work
                demonstrates binderctional pipes
                input looks like number op number which
                tinybc converts into number \n number \n op \n p
                and passws result back to stdout
...
int main(void)
{
    ...
    if( (pid = fork()) == -1)
        oops("cannot fork", 2);
    if (pid == 0)                   /*  child process, child is dc   */
        be_dc(todc, fromdc);
    else {  
        be_bc(todc, fromdc);        /*  parent process, parent is ui    */
        wait(NULL);                 /*  wait for child process          */
    }
}

/* set up stdin and stdout, then execl dc   */
/* child process에서 수행하는 함수 */
void be_dc(int in[2], int out[2])      // todc -> in, from dv ->out 
{
    /*  setup stdin from pipein */
        if(dup2(in[0], 0) == -1)    /* copy read end to 0   */
            oops("dc : cannot redirect stdin", 3);
        close(in[0]);   /* moved to fd 0    */
        close(in[1]);   /* won't wirte here */
    ...
    /* now execl do with the - option   */
        execlp("dc", "dc", "-", NULL);  // 성공할 경우
        oops("Cannot run dc", 5);       // 실패할 경우
}

/*
    read from stdin and convert in to RPN, send down pipe
